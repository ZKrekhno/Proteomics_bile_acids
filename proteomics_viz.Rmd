...
#Create labels
dummy_labels <- glue_data(dummy_df, 
                          "<span style='color: {if_else(Direction == 'Down-regulated', 'red', 'blue')}'>{clean_ID}</span>")
names(dummy_labels) <- dummy_df$clean_ID
...
ggplot(dummy_df, aes(x = OddsRatio, y = clean_ID )) +
  ...
  scale_y_discrete(labels = dummy_labels) +
